...
class CultureOracle(OracleMasterTemplate):

    def runOracle(self):
        orm = ORM()

        stm = """SELECT * FROM metaTag where tagName='openpipe_canonical_culture' and status='formatted' group by value;"""
        ...
        for r in res['data']:
            ...
            metaTagValue=cf.removePrantesis(metaTagValue)
            # metaTagValue=metaTagValue.split(",")[0]
