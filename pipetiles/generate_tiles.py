...
# CAUTION!! This program will remove any data in the output directory!!
...
face_map = {"b":"0", "l":"1", "f":"2", "r":"3", "u":"4", "d":"5"}
alphabetical_order = {"b":"0", "d":"1", "f":"2", "l":"3", "r":"4", "u":"5"}

def generate_dir_tree(output_dir, levels, faces):
    cmd = "rm -rf {}".format(output_dir)
    os.system(cmd)
    os.mkdir(output_dir)
...
def generate_face_tiles(output_dir, levels, face, imgprefix):
    filename = imgprefix + "_" + face_map[face] + ".png"
    print(filename)
    ...
    for level in reversed(range(1, levels+1)):
        ...
        for row in range(0,h):
            for col in range(0,h):
                ...
                outfile = output_dir + "/" + str(level) + "/" + face + "/" + str(col) + "/" + str(row) + ".png"
                generate_tile(img, row, col, tile_h, tile_w, outfile)
        img = cv2.resize(img, None, fx=0.5, fy=0.5)
    offset = int(alphabetical_order[face])*256
    preview[offset:offset+256, 0:256] = img
...
preview = np.zeros((6*256, 1*256, 3), dtype=np.uint8)

for face in faces:
    generate_face_tiles(output_dir, levels, face, imgprefix)

cv2.imwrite(output_dir + "/preview.png", preview)
print("Done!")
